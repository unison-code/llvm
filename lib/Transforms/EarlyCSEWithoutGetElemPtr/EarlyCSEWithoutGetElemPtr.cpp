//===- EarlyCSEWithoutGetElemPtr.cpp - Simple and fast CSE pass ----------------------------===//
//
...
class EarlyCSEWithoutGetElemPtrPass {
public:
...
#define DEBUG_TYPE "early-cse-without-getelemptr"
...
//===----------------------------------------------------------------------===//
// EarlyCSEWithoutGetElemPtr implementation
//===----------------------------------------------------------------------===//
...
/// cases so that instcombine and other passes are more effective. It is
/// expected that a later pass of GVN will catch the interesting/hard cases.
class EarlyCSEWithoutGetElemPtr {
public:
  ...
  EarlyCSEWithoutGetElemPtr(const TargetLibraryInfo &TLI, const TargetTransformInfo &TTI,
...
bool EarlyCSEWithoutGetElemPtr::processNode(DomTreeNode *Node) {
      ...
      if (BI->isConditional())
        if (auto *CondInst = dyn_cast<Instruction>(BI->getCondition()))
          if (SimpleValue::canHandle(CondInst)) {
            ...
            DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr CVP: Add conditional value for '"
  ...
  for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E;) {
    ...
    if (isInstructionTriviallyDead(Inst, &TLI)) {
      DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr DCE: " << *Inst << '\n');
    ...
    if (match(Inst, m_Intrinsic<Intrinsic::assume>())) {
      DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr skipping assumption: " << *Inst << '\n');
      continue;
    }
    ...
    if (Value *V = SimplifyInstruction(Inst, DL, &TLI, &DT, &AC)) {
      // Forbid simplifications resulting in a getelementptr
      if (dyn_cast<GetElementPtrInst>(V)) {
        DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr Simplify: " << *Inst << "  to: " << *V << '\n');
    ...
    if (SimpleValue::canHandle(Inst)) {
      ...
      if (Value *V = AvailableValues.lookup(Inst)) {
        DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr CSE: " << *Inst << "  to: " << *V << '\n');
        Inst->replaceAllUsesWith(V);
        Inst->eraseFromParent();
        Changed = true;
        ++NumCSE;
    ...
    if (MemInst.isValid() && MemInst.isLoad()) {
      ...
      if (InVal.Data != nullptr && InVal.Generation == CurrentGeneration &&
        ...
        if (Op != nullptr) {
          DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr CSE LOAD: " << *Inst
                       << "  to: " << *InVal.Data << '\n');
          if (!Inst->use_empty())
    ...
    if (CallValue::canHandle(Inst)) {
      ...
      if (InVal.first != nullptr && InVal.second == CurrentGeneration) {
        DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr CSE CALL: " << *Inst
                     << "  to: " << *InVal.first << '\n');
        if (!Inst->use_empty())
    ...
    if (MemInst.isValid() && MemInst.isStore()) {
      ...
      if (InVal.Data &&
        ...
        DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr DSE (writeback): " << *Inst << '\n');
        Inst->eraseFromParent();
        Changed = true;
        ++NumDSE;
      ...
      if (MemInst.isValid() && MemInst.isStore()) {
        ...
        if (LastStore) {
          ...
          if (LastStoreMemInst.isMatchingMemLoc(MemInst)) {
            DEBUG(dbgs() << "EarlyCSEWithoutGetElemPtr DEAD STORE: " << *LastStore
                         << "  due to: " << *Inst << '\n');
...
bool EarlyCSEWithoutGetElemPtr::run() {
...
PreservedAnalyses EarlyCSEWithoutGetElemPtrPass::run(Function &F,
  ...
  EarlyCSEWithoutGetElemPtr CSE(TLI, TTI, DT, AC);
...
/// cases so that instcombine and other passes are more effective. It is
/// expected that a later pass of GVN will catch the interesting/hard cases.
class EarlyCSEWithoutGetElemPtrLegacyPass : public FunctionPass {
public:
  static char ID;

  EarlyCSEWithoutGetElemPtrLegacyPass() : FunctionPass(ID) {
...
char EarlyCSEWithoutGetElemPtrLegacyPass::ID = 0;
static RegisterPass<EarlyCSEWithoutGetElemPtrLegacyPass>
Y("early-cse-without-getelemptr", "EarlyCSEWithoutGetElemPtr Pass");
