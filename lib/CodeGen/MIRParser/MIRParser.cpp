...
bool MIRParserImpl::setMachineFunction(MachineFunction &MF) {
  // Parse the yaml.
  yaml::MachineFunction YamlMF;
  yaml::EmptyContext Ctx;
  yaml::yamlize(In, YamlMF, false, Ctx);
  if (In.error())
    return true;
  return initializeMachineFunction(YamlMF, MF);
}
...
bool MIRParser::setMachineFunction(MachineFunction &MF) {
  return Impl->setMachineFunction(MF);
}
